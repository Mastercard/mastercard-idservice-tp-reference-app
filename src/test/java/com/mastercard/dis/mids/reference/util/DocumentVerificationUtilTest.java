...
class DocumentVerificationUtilTest {
    ...
    @Test
    void setupUserIdentityTokens_test() {
        when(apiClientMock.addDefaultHeader(anyString(), anyString()))
                .thenReturn(apiClientMock);

        this.documentVerificationUtil.setupUserIdentityTokens();

        verify(apiClientMock, times(1)).addDefaultHeader(anyString(), anyString());
    }
